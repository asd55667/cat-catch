...
$("#exportOptions").bind("click", function () {
    chrome.storage.sync.get(null, function (items) {
        ...
        let date = new Date();
        const filename = `cat-catch-custom-${chrome.runtime.getManifest().version}-${date.getFullYear()}${appendZero(date.getMonth() + 1)}${appendZero(date.getDate())}T${appendZero(date.getHours())}${appendZero(date.getMinutes())}.txt`;
        if (G.isFirefox) {
